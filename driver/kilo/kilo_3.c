...
struct nand_controller_chip kilo_controller_chip = {
	...
	.first_storage = NULL, /* BUG: no storage chips */
	.last_storage = NULL   /* BUG: no storage chips */
};
...
int exec_op(struct nand_operation *commands)
{
	...
	for (int i = 0; i < commands->ninstrs; i++) {
		struct nand_op_instr command = commands->instrs[i];
		switch (command.type)
		{
...
struct nand_driver get_driver()
{
	struct nand_driver ret = {
		.type = NAND_EXEC_OP,
		.operation.exec_op = kilo_controller_chip.exec_op,
	};
	return ret;
}
