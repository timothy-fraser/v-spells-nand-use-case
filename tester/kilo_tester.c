...
{
	struct nand_storage_chip init_storage = {
	...
	struct nand_controller_chip init_controller = {
	...
	struct nand_device init_device = {
	...
	struct nand_device *dib_old = &init_device;
	struct nand_device *dib_new = NULL;

	init_storage.controller= &init_controller;

	puts("Test: update Device Information Base (DIB):");	
	dib_new = init_framework(ioregister, dib_old);
	...
	puts("Confirmed DIB well-formed after update.\n");
